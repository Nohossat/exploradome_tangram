    ...
    if args.mode:
        ...
        if args.mode.endswith((".jpg", ".png")):
            ...
            assert os.path.exists(
                args.mode), "the file doesn't exist - try with another file"
            ...
            tangram_game_live_test(video=int(args.mode),
                                   side=args.side, prepro=preprocess_img_2)
        else:
            raise Exception(
                "the mode isn't valid - pass a valid image/video path or a webcam stream")

    if args.metrics:
        assert os.path.exists(
            args.metrics), "the folder doesn't exist - try with another one"
