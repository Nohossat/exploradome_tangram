...
def tangram_game(hu_moments_dataset='data/hu_moments.csv', side=None, video=0, image=False, prepro=False):
    ...
    if not isinstance(video, bool):
        ...
        while(cap.isOpened()):
            ...
            predictions, cnts = get_predictions(image, hu_moments, target, side = side,prepro=False) # retourne cnts so you can print them too
            print(predictions)
...
def tangram_game_live_test(side=None, video=0,prepro=False):
    ...
    while(cap.isOpened()):
        ...
        predictions, cnts = get_predictions(image, hu_moments, target, side = side,prepro=False)
